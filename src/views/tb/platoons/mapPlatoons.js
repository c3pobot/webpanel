...
function getUnits(baseId, members = [], playerUnitCount = {}, preAssigned, maxUnit = 10){
  ...
  return res.filter(x=>x.rosterUnit?.filter(u=>u.definitionId?.startsWith(`${baseId}:`))?.length > 0).map(x=>{
    ...
    return { player: x.name, playerId: x.playerId, baseId: baseId, relicTier: +(unit?.relic?.currentTier || 0), rarity: +(unit.currentRarity || 0), level: +(unit.currentLevel || 0), tier: +(unit.currentTier || 0), sort: +(unit.relic?.currentTier || 0) + +(unit.currentTier || 0) }
  })
}
...
function mapUnits(unit = {}, platoon = {}, squad = {}, data = {}){
  ...
  if(assigned?.length > 0){
    for(let i in assigned){
      ...
      //avaliableUnits = avaliableUnits.filter(x=>x.playerId !== assigned[i])
    }
  }
  if(unit.count < 0) unit.count = 0
  ...
  if(avaliableUnits?.length > 0) avaliableUnits = sorter(avaliableUnits, 'aesc', 'sort')
...
function mapPlatoonDef(pDef = {}, data = {}){
  ...
  squads = sorter(squads, 'desc', 'points')
  ...
  if(tempPlatoon.squads?.length > 0) tempPlatoon.squads = sorter(tempPlatoon.squads, 'asce', 'num')
...
function getPlatoonConfig(platoonConfig = [], pDef = []){
  ...
  for(let i in platoonConfig){
    ...
    if(platoonConfig[i].sort) tempObj.sort = platoonConfig[i].sort
    res.push(tempObj)
  }
  return res
}

export default function MapPlatoons(pDefinition = [], members = [], platoonIds, tbDay = 1){

  //let data = { units: {}, preAssigned: getPreAssigned(platoonIds), platoonIds: platoonIds, members: sorter(members, 'asce', 'playerId') }, res = []
  let data = { units: {}, preAssigned: getPreAssigned(platoonIds), platoonIds: platoonIds, members: members }, res = []
  let pDef = getPlatoonConfig(platoonIds || pDefinition.filter(x=>x.phase === 'P'+tbDay), pDefinition)
  pDef = sorter(pDef, 'asce', 'sort')
  for(let i in pDef){
    let tempPlatoon = mapPlatoonDef(pDef[i], data)
    res.push(tempPlatoon)
  }
  return res
}
